...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (data.street !== undefined || data.number !== undefined) {
      const street = data.street !== undefined ? clean(data.street, 160) : undefined
      const number = data.number !== undefined ? clean(data.number, 32) : undefined
      ...
      if (existingAddress) await db.update(clientAddresses).set({ ...(street !== undefined ? { street } : {}), ...(number !== undefined ? { number } : {}), updatedAt: new Date() }).where(eq(clientAddresses.id, existingAddress.id))
      else await db.insert(clientAddresses).values({ clientId: id, street: street || 'Endereço a confirmar', number: number || 'S/N', neighborhood: 'Bairro a confirmar', city: 'Niterói', state: 'RJ', isMain: true })
    }
    await db.insert(sofiaEvents).values({ senderPhone: String((rawBody as Record<string, unknown>).senderPhone), idempotencyKey: correlationId, rawPayload: { action, clientId: id, fields: Object.keys(updates).filter(key => key !== 'updatedAt').concat(data.street !== undefined ? ['street'] : [], data.number !== undefined ? ['number'] : []) }, intentDetected: 'crm_update_client', status: 'PROCESSED' })
